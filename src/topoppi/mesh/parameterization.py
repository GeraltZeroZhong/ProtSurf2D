...
class Parameterizer:
    ...
    def flatten_patch(self, mesh: trimesh.Trimesh, method: str | None = None, return_info: bool = False):
        ...
        try:
            ...
            if len(mesh.vertices) < config.min_vertices or len(mesh.faces) == 0:
        ...
        if chi != config.expected_euler_characteristic or n_boundary_loops != config.expected_boundary_loops:
            ...
            if disk_mesh is not None and len(disk_mesh.faces) > 0:
                ...
                f = np.ascontiguousarray(mesh.faces, dtype=np.int32)
                chi, n_edges, n_boundary_loops = Parameterizer._topology_stats(f, len(v))
                diag["diskification_success"] = bool(
                    chi == config.expected_euler_characteristic
                    and n_boundary_loops == config.expected_boundary_loops
                )
                logger.info(
        ...
        # 5. Harmonic Parameterization
        # Triggered only in auto/harmonic modes.
    ...
    def _flatten_harmonic(v, f, bnd):
        """
        Harmonic method: map boundary to circle and minimize Dirichlet energy.
        """
        try:
    ...
    def _normalize_uv(uv):
        ...
        scale = uv_max - uv_min
        scale[scale < ParameterizationConfig().uv_epsilon] = 1.0 
